...
class StartEndDataset(Dataset):
    def __init__(self,dset_name, data_path, query_image_patches_path_or_handler,desc_bert_path_or_handler, 
                 sub_bert_path_or_handler, max_desc_len,max_ctx_len, vid_feat_path_or_handler, clip_length, 
                 query_image_patches,query_image_size,ctx_mode="video", normalize_vfeat=True,normalize_tfeat=True,
                 normalize_query_image_feat=True,h5driver=None, data_ratio=1.0
                 ):
        self.dset_name = dset_name
        self.data_path = data_path
        self.data_ratio = data_ratio

        self.query_image_patches_path_or_handler = query_image_patches_path_or_handler
        self.query_image_patches = query_image_patches
        ...
        self.use_query_image =  query_image_patches_path_or_handler is not None

        if self.use_video:
            logger.info("Loading video features from {}.............".format(vid_feat_path_or_handler))
        ...
        if self.use_query_image:
            logger.info("Loading query image features from {}............".format(query_image_patches_path_or_handler))
            if isinstance(query_image_patches_path_or_handler, h5py.File):
                self.query_image_h5 = query_image_patches_path_or_handler
            else:
                self.query_image_h5 = h5py.File(query_image_patches_path_or_handler, "r", driver=h5driver)
        else:
            self.query_image_h5 = None
        # desc embbeding
        logger.info("Loading desc features from {}............".format(desc_bert_path_or_handler))
        
        if isinstance(desc_bert_path_or_handler, h5py.File):
            self.desc_bert_h5 = desc_bert_path_or_handler
        else:
            print("desc_bert_path_or_handler:{}".format(desc_bert_path_or_handler))
            self.desc_bert_h5 = h5py.File(desc_bert_path_or_handler, "r", driver=h5driver)

        if self.use_sub:
            # import os
            # print(os.path.exists(desc_bert_path_or_handler))
            # exit()
            logger.info("Loading sub features from {}............".format(sub_bert_path_or_handler))
        ...
        self.normalize_query_image_feat = normalize_query_image_feat
    ...
    def __getitem__(self, index):
        ...
        if(self.use_query_image):
            model_inputs["query_image_feat"] = self.get_query_feat_by_desc_id(meta["desc_id"],type="image")
        else:
            model_inputs["query_image_feat"] = torch.zeros((2, 2))

        ctx_l = 0
        if self.use_video:
            #vid_feat shape: (n_clip, D_video)
            video_feat = uniform_feature_sampling(self.vid_feat_h5[meta['vid_name']][:], self.max_ctx_len)
            if self.normalize_vfeat:
                # logger.info("normalize video feat")
        ...
        if self.use_sub:  # no need for ctx feature, as the features are already contextualized
            sub_feat = uniform_feature_sampling(self.sub_bert_h5[meta["vid_name"]][:], ctx_l)
            if self.normalize_tfeat:
                # logger.info("normalize sub feat")
                sub_feat = l2_normalize_np_array(sub_feat)
                
            sub_feat = sub_feat[:ctx_l]
        ...
        if self.use_video and self.use_tef:  # (N_clips, D + 2)
            # print(model_inputs["video_feat"].shape)
            # print(tef_feat.shape)
            
            if(model_inputs["video_feat"].shape[0]!=tef_feat.shape[0]):
                print("video_feat shape:{}".format(model_inputs["video_feat"].shape))
                print("tef_feat shape:{}".format(tef_feat.shape))
                print("vid_name:{}".format(meta["vid_name"]))
            try:
                model_inputs["video_feat"] = torch.cat([model_inputs["video_feat"], tef_feat], dim=1)
            except:
                print(model_inputs["video_feat"].shape,tef_feat.shape,)
    ...
    def get_query_feat_by_desc_id(self, desc_id,type:str="desc"):
        #default:30
        if(type=="desc"):       
            query_feat = self.desc_bert_h5[str(desc_id)][:self.max_desc_len]
            if self.normalize_tfeat:
                # logger.info("normalize query feat")
                query_feat = l2_normalize_np_array(query_feat)
            return torch.from_numpy(query_feat)
        elif(type=="image"):
            query_feat = self.query_image_h5[str(desc_id)][:self.query_image_patches+1]
            if self.normalize_query_image_feat:
                # logger.info("normalize query image feat")
                query_feat = l2_normalize_np_array(query_feat)
                # print("normalized query image feat:{}".format(query_feat.shape))
            return torch.from_numpy(query_feat)
        else:
            raise ValueError("type must be 'desc' or 'image'")
...
class StartEndEvalDataset(Dataset):
    ...
    def __init__(self, dset_name, eval_split_name, data_path=None, desc_bert_path_or_handler=None, max_desc_len=None,
                 query_image_patches_path_or_handler=None,query_image_patches=None,query_image_size=None,
                 ...
                 data_ratio=1.0, normalize_vfeat=True, normalize_tfeat=True,normalize_query_image_feat=True):
        self.dset_name = dset_name
        ...
        self.max_desc_len = max_desc_len #30
        self.max_ctx_len = max_ctx_len #128
        self.data_path = data_path #data/tvr_val_release.jsonl
        if isinstance(desc_bert_path_or_handler, h5py.File):
            self.desc_bert_h5 = desc_bert_path_or_handler
        else:
            if(desc_bert_path_or_handler is None):
                self.desc_bert_h5 = None
            else:
                self.desc_bert_h5 = h5py.File(desc_bert_path_or_handler, "r", driver=h5driver)

        video_data = load_json(video_duration_idx_path)[self.eval_split_name] 
        #vid2dur.keys():['test_challenge', 'test_public', 'train', 'val']
        ...
        if self.use_video:
            if isinstance(vid_feat_path_or_handler, h5py.File):
                ...
                if(vid_feat_path_or_handler is None):
                    self.vid_feat_h5 = None
                else:
                    self.vid_feat_h5 = h5py.File(vid_feat_path_or_handler, "r", driver=h5driver)
        ...
        if self.use_sub:
            if isinstance(sub_bert_path_or_handler, h5py.File):
                ...
                if(sub_bert_path_or_handler is None):
    ...
    def __len__(self):
        if self.data_mode == "context":
            return len(self.video_data) #[{vid_name, duration}, ...]
        else:
            return len(self.query_data)#[{json},]
...
def start_end_collate(batch):
    batch_meta = [e["meta"] for e in batch] #(N, ),each one is a dict
    ...
    # sub_feat = batched_data.get("sub_feat")[0]
    fixed_length = 128
...
def prepare_batch_inputs(batched_model_inputs, device, non_blocking=False):
    ...
    for k, v in batched_model_inputs.items():
        # query_feat,video_feat,sub_feat
        if "feat" in k:
