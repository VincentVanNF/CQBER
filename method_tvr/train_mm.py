...
from model_mm import ReLoCLNet
from start_end_dataset import StartEndDataset, start_end_collate, StartEndEvalDataset, prepare_batch_inputs
...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader), desc="Training:{} Iteration".format(training), total=num_training_examples):
...
def train(model, train_dataset, train_eval_dataset, val_dataset, opt):
    ...
    if opt.device.type == "cuda":
        ...
        logger.info("USE GPU: {} for training".format(opt.device))
        model.to(opt.device)
        if len(opt.device_ids) > 1:
            logger.info("Use multi GPU:{} ".format(os.environ['CUDA_VISIBLE_DEVICES']))
    ...
    # Prepare optimizer
    # param_optimizer: name, param metrix
    ...
    # filename: latest_tvr_val_predictions_SVMR_VCMR_VR.json
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None:
            with torch.no_grad():
                ...
                metrics_no_nms, metrics_nms, latest_file_paths = eval_epoch(model, val_dataset, opt, save_submission_filename, 
            ...
            stop_metric_names = ["r1"] if opt.stop_task == "VR" else ["0.3-r1","0.5-r1", "0.7-r1"]
...
def set_none(opt):
    opt.vid_feat_path = None
    opt.desc_bert_path = None
    opt.sub_bert_path = None
...
def start_training():
    ...
    train_dataset = StartEndDataset(
        ...
        query_image_patches_path_or_handler=opt.query_image_patches_path,
        
        max_desc_len=opt.max_desc_l,
        max_ctx_len=opt.max_ctx_l,
        query_image_patches = opt.query_image_patches, # 49
        query_image_size = opt.query_image_size, # 224
        ...
        normalize_query_image_feat=not opt.no_norm_query_image_feat
        )

    logger.info("Prepare eval_train dataset................................")
    if opt.eval_path is not None:
        ...
        logger.info("Prepare eval dataset................................")
        eval_dataset = StartEndEvalDataset(
            ...
            desc_bert_path_or_handler=train_dataset.desc_bert_h5,
            sub_bert_path_or_handler=train_dataset.sub_bert_h5 if "sub" in opt.ctx_mode else None,
            query_image_patches_path_or_handler=train_dataset.query_image_h5,
    ...
    model_config = EDict(
        PLUG=opt.PLUG,
        query_image_patches = opt.query_image_patches, # 49
        query_image_size = opt.query_image_size, # 768
        use_query_image = False, 
        visual_input_size=opt.vid_feat_size, # 3072=2048+1024
        sub_input_size=opt.sub_feat_size,  # for both desc and subtitles # 768
        query_input_size=opt.q_feat_size,  # for both desc and subtitles # 768
        hidden_size=opt.hidden_size,  # hidden dimension, 384
        conv_kernel_size=opt.conv_kernel_size, # 5
        conv_stride=opt.conv_stride, # 1
        max_ctx_l=opt.max_ctx_l, # 128
        
        max_mutimodal_l=opt.query_image_patches + opt.max_desc_l,
        
        max_desc_l=opt.max_desc_l, # 30
        input_drop=opt.input_drop, # 0.1
        drop=opt.drop, #0.1
        n_heads=opt.n_heads,  # self-att heads, 8
        initializer_range=opt.initializer_range,  # for linear layer, 0.02
        ctx_mode=opt.ctx_mode,  # video, sub or video_sub: video_sub_tef
        margin=opt.margin,  # margin for higing loss 0.1
        ranking_loss_type=opt.ranking_loss_type,  # loss type, 'hinge' or 'lse'
        
        lw_neg_q=opt.lw_neg_q,  # loss weight for neg. query and pos. context:1
        lw_neg_ctx=opt.lw_neg_ctx,  # loss weight for pos. query and neg. context :1
        lw_fcl=opt.lw_fcl,  # loss weight for frame level contrastive learning:0.03
        lw_vcl=opt.lw_vcl,  # loss weight for video level contrastive learning:0.03
        ...
        ) #20:hard negatives are still sampled, but from a harder pool.
