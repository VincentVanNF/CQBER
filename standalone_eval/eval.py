...
def pad_sequences_1d_np(sequences, dtype=np.float32):
    ...
    # extra_dims: 6: [vid_name_idx (int), st (float), ed (float), is_vid_name_match (bool), iou_scores>=iou_thd0:0.5 (bool), iou_scores>=iou_thd1:0.7 (bool)]
    extra_dims = sequences[0].shape[1:]  # the extra dims should be the same for all elements
    lengths = [len(seq) for seq in sequences] # 每个query的预测结果的个数
    
    assert "numpy" in str(dtype), "dtype and input type does not match"
    # padded_seqs: (n_desc, max_length, 6)
    ...
    for idx, seq in enumerate(sequences):
        ...
        mask[idx, :end] = 1
    # mask: (n_desc, max_length)，1表示有效，0表示无效，代表预测结果有效的mask
...
def eval_by_task_type(moment_predictions, video2idx, ground_truth,
    ...
    # 遍历一个task中的 val jsonl文件中的desc_id，k为desc_id，gt_item为desc_id对应的json
    for k, gt_item in tqdm(gt_by_desc_id.items(), desc="Loop over moments", leave=False):
        if not match_number and k not in predictions_by_desc_id:
            continue
        # 一个query对应的预测结果的前3个元素:[[vid_name_idx (int), st (float), ed (float)],..]
        ...
        '''
        pred_info_matrix:100个
            [
                [vid_name_idx (int), st (float), ed (float), is_vid_name_match (bool), iou_scores>=iou_thd0:0.5 (bool), iou_scores>=iou_thd1:0.7 (bool)],
                .....
            ]
        
        pred_info_matrix_collection：
        N_query个pred_info_matrix:[
            [
                [vid_name_idx (int), st (float), ed (float), is_vid_name_match (bool), iou_scores>=iou_thd0:0.5 (bool), iou_scores>=iou_thd1:0.7 (bool)],
                .....
            ],
    ...
    # column header [vid_name_idx (int), st (float), ed (float), is_vid_name_match (bool),iou_scores>=iou_thd0 (bool), iou_scores>=iou_thd1 (bool)]
    ...
    '''
        metrics:{
            "iou_thd-rk":percentage,
            ....
        }
        metrics_by_type:{
            "desc_type_ratio": "v {} t {} vt {}",
            "desc_type-iou_thd-rk":percentage,
            .... 
        }
    '''
    metrics = OrderedDict()
    ...
    if task_type == "VCMR":
        for iou_idx, iou_thd in enumerate(iou_thds):
            # 所有desc_num的每个query中的预测结果的iou是否>=iou_thd,shape:(n_desc, n_pred)
            ...
            '''
                对于VCMR:
                每个query 在 video * L * L个预测结果中，是否有一个预测结果的iou>=iou_thd且vid_name匹配则代表预测正确
                recall_k: 在k个预测结果中，是否有一个预测结果的iou>=iou_thd且vid_name匹配则为预测正确
                recall_k计算的是 预测结果为正样本的占所有预测结果的比例
            '''
            for k in recall_topks:
                metrics["{}-r{}".format(iou_thd, k)] = \
                    get_rounded_percentage(np.mean(np.sum(iou_corrects[:, :k], axis=1) >= 1)) #>=1代表每一个query中是否有一个预测结果的iou>=iou_thd，即预测结果是否正确
        if use_desc_type:
            for desc_type in desc_type2idx:
                type_corrects = desc_types == desc_type2idx[desc_type]  # (n_desc)
                n_desc_in_type = np.sum(type_corrects)  # (n_desc),预测中每一个desc_type的数量
                for iou_idx, iou_thd in enumerate(iou_thds):
                    # (n_desc, n_pred)
        ...
        vid_name_matched = pred_info_matrix_collection[:, :, 3].astype(np.bool_)  # (n_desc, n_pred)
        ...
        for iou_idx, iou_thd in enumerate(iou_thds):
            ...
            '''
                SVMR-recall-k:
                在每个desc query的在gt video中的max_pred_per_query个预测结果中，
                是否有一个预测结果的iou>=iou_thd且vid_name匹配则代表预测正确。
                recall的百分比为：预测正确的数量/总的desc query数量
            '''
            for k in recall_topks:
                ...
                # 这个比例的意义是在所有的预测结果中，有多少个预测结果的iou>=iou_thd且vid_name匹配
        if use_desc_type:
            for desc_type in desc_type2idx:
                type_corrects = desc_types == desc_type2idx[desc_type]  # (n_desc)
        ...
        '''
            VR-recall-k:
            在每个desc query的在所有video的max_pred_per_query个预测结果中，
            是否有一个预测结果的vid_name匹配则代表预测正确。
            recall的百分比为：预测正确的数量/总的desc query数量
        '''
        for k in recall_topks:
            metrics["r{}".format(k)] = \
                get_rounded_percentage(np.mean(np.sum(vid_name_matched[:, :k], axis=1) >= 1)) #>=1代表每一个query中是否有一个预测结果的vid_name匹配，即预测结果是否正确
        if use_desc_type:
            for desc_type in desc_type2idx:
                type_corrects = desc_types == desc_type2idx[desc_type]  # (n_desc)
                n_desc_in_type = np.sum(type_corrects)  # (n_desc)
...
def eval_retrieval(submission, ground_truth, iou_thds=(0.3,0.5, 0.7), verbose=True, match_number=True, use_desc_type=True):
    ...
    metrics_raw_dict = {}
    # task_type:["VCMR", "SVMR", "VR]
    '''
    ground_truth:[json, json, json, ...]  #in val jsonl file
    '''
    for task_type in submitted_task_types:
        ...
        metrics, metrics_by_type = eval_by_task_type(
            ...
            iou_thds=iou_thds, recall_topks=(1,5,10, 100),  # (1, 5, 10, 20, 50, 100),
