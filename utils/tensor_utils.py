...
def pad_sequences_1d(sequences, dtype=torch.long, device=torch.device("cpu"), fixed_length=None):
    ...
    extra_dims = sequences[0].shape[1:]  # the extra dims should be the same for all elements,hidden_size
    lengths = [len(seq) for seq in sequences] #(batch_size, seq_len)
    ...
    if isinstance(sequences[0], torch.Tensor):
        ...
        # padded_seqs: (batch_size, max_length, hidden_szie)
    ...
    for idx, seq in enumerate(sequences):
        ...
        mask[idx, :end] = 1
    # padded_seqs: (batch_size, max_length, hidden_szie)
    # mask: (batch_size, max_length) 1 indicate valid, 0 otherwise
...
def top_n_array_2d(array_2d, top_n):
    ...
    ranked by the value，
    return:[
        row_indx,col_indx,sorted_value \n
        row_indx,col_indx,sorted_value\n
        ........\n
    ]
    """
